...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='/eecf/cbcsl/data100b/Chenqi/data/flower/', type=str, help='Dir of the training dataset')
    ...
    if 'MNIST' not in dataset:
        ...
        train_transform = transforms.Compose([
            transforms.RandomResizedCrop(img_size), # NOT use random crop! use resize & center crop!!!
            transforms.RandomHorizontalFlip(p=0.5), # for FLOWER & MNIST: NOT do this!
            transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8), # NOT jitter that much for FLOWER!! Add Gaussian blurring.
            transforms.RandomGrayscale(p=0.2), # maybe also add affain warping?
            transforms.ToTensor(),
    ...
    train_data = utils_chenqi.MyCustomDataset(root=data_dir, label_file=label_file, transform=train_transform)
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=16, pin_memory=True,
                              drop_last=True)
    """
    # original old code:
    ...
    memory_data = utils_chenqi.MyCustomDataset(root=data_dir, label_file=label_file, transform=test_transform)
    memory_loader = DataLoader(memory_data, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
    ...
    test_data = utils_chenqi.MyCustomDataset(root=data_dir, label_file=label_file, transform=train_transform) # make the testing set to be the transformed original image!!!
    ...
    for epoch in range(1, epochs + 1):
        ...
        # newly modified:
        data_frame.to_csv('results/' + dataset + '/' + '{}_statistics.csv'.format(save_name_pre), index_label='epoch')
        ...
        if test_acc_1 > best_acc:
            best_acc = test_acc_1
            # newly modified:
            torch.save(model.state_dict(), 'results/' + dataset + '/' + '{}_model.pth'.format(save_name_pre))
        """
        # new code: save all the models!!! (while also keep track on the "best" model):
        torch.save(model.state_dict(), 'results/' + dataset + '/' + 'epoch{}'.format(epoch) + '_{}_model.pth'.format(save_name_pre))
        if test_acc_1 > best_acc:
            best_acc = test_acc_1
            # newly modified:
            torch.save(model.state_dict(), 'results/' + dataset + '/' + 'best_{}_model.pth'.format(save_name_pre))
