    ...
    def add_image(self, img):
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            ...
            ex = tf.train.Example(features=tf.train.Features(feature={
                ...
                'data': tf.train.Feature(bytes_list=tf.train.BytesList(value=[quant.tobytes()]))})) # DeprecationWarning: tostring() is deprecated. Use tobytes() instead
...
# this func is modified by Chenqi:
def create_from_images(tfrecord_dir, image_dir, shuffle, resolution, random_subset):# newly added arg resolution=128, random_subset
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    
    # modified: if random_subset != 0, randomly selected a subset of random_subset number of imgs from image_filenames
    if random_subset != 0:
        # random sampling without replacement
        random.seed(4)
        image_filenames = random.sample(image_filenames, random_subset)
    ...
    """
    # modify: so that img can be of any size
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            ...
            #"""
            # Below code is NOT used when images are grayscale and don't need to crop!:
            channels = img.shape[2] if img.ndim == 3 else 1 # newly added
            # modify: newly added: referenced from func create_lsun()
            # center crop and resize so that img can be of any size
            crop = np.min(img.shape[:2])
            img = img[(img.shape[0] - crop) // 2 : (img.shape[0] + crop) // 2, (img.shape[1] - crop) // 2 : (img.shape[1] + crop) // 2]
            
            img = PIL.Image.fromarray(img, 'RGB')
            #if channels == 3: # RGB img
            #    img = PIL.Image.fromarray(img, 'RGB')
            #elif channels == 1: # grayscale img
            ...
            """
            # for degbu:
            print('img.shape = ' + str(img.shape)) # (3, 128, 128) for CelebA(RGB); (1, 128, 128) for MNIST(grayscale)
            assert(False)
            """
            tfr.add_image(img)
...
def execute_cmdline(argv):
    ...
    # modified: newly added by Chenqi:
    p.add_argument(     '--resolution',     help='Output resolution (default: 128)', type=int, default=128)
    p.add_argument(     '--random_subset',     help='Number of imgs used in training set (default: 0 means using all imgs; else is the number of subset imgs)', type=int, default=0)
