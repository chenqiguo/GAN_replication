...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 23 13:53:15 2021

@author: guo.1648
"""

# referenced from biggan inception_utils.py,
# and simCLR NNquery_simCLR_myTest.py.

# My 1st try to use inception v3 features to do NN query.
...
def NNquery_func(net, train_loader, test_loader):
    ...
    with torch.no_grad():
        ...
        for i, (data, _target) in enumerate(train_loader, 0):
            ...
            feature_bank.append(pool_val.cpu()) #.detach().numpy()
        ...
        for i, (data, _target) in enumerate(test_loader, 0): # only 1 img at a time (since batch_size=1)
            ...
            #print()
            feature_arr = torch.Tensor.cpu(pool_val).detach().numpy()[0] # of shape (2048,)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='NNquery inception v3')
    parser.add_argument('--dataset', default='FLOWER_128_sub1000', type=str, help='Name of the corresponding gan experiment, eg, FLOWER_128_sub1000')
    parser.add_argument('--data_dir', default='/eecf/cbcsl/data100b/Chenqi/BigGAN-PyTorch/imgs/FLOWER_128_sub1000/', type=str, help='Dir of the original training dataset')
    parser.add_argument('--gan_dir', default='/eecf/cbcsl/data100b/Chenqi/gan_results_for_presentation/biggan/NNquery_inception_v3/FLOWER_128_sub1000/Itr38950/view_sampleSheetImgs/', type=str, help='Dir of the gan generated dataset, as testing')
    parser.add_argument('--result_dir', default='/eecf/cbcsl/data100b/Chenqi/gan_results_for_presentation/biggan/NNquery_inception_v3/FLOWER_128_sub1000/Itr38950/', type=str, help='Dir of the matching results')
    ...
    # load model:
    inception_model = load_inception_net(parallel=True)
    """
    ...
    # do our NN query using the feature output by the model:
    NNquery_func(inception_model, train_loader, test_loader)
