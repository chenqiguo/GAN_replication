...
def make_dataset(dir, class_to_idx): # load all legal image file names and corresponding labels in dir
  images = []
...
# modify: func added by Chenqi:
def make_dataset_subset(root_real, class_to_idx, subset_size):
  # referenced from above func make_dataset().
  # Args:
  #   root_real: e.g., 'data/flower' --> SAME as arg root in func make_dataset()
  #   class_to_idx: e.g., {'jpg': 0} --> SAME as arg root in func make_dataset()
  #   subset_size: e.g., 1000 --> NEWLY added!!!: number of subset images we want
  assert(isinstance(subset_size, int))
  images = [] # all images: SAME as images in func make_dataset()
  dir = root_real
  # below: SAME code as in func make_dataset()
  dir = os.path.expanduser(dir)
  for target in tqdm(sorted(os.listdir(dir))):
    d = os.path.join(dir, target)
    if not os.path.isdir(d):
  ...
  #print('$$$$$$$$$$$$$$$$$$$$$$$$')
  #print(images)
  #print(len(images)) # 8189
  #print(images[0]) # ('data/flower/jpg/image_00001.jpg', 0)
  
  # newly added code:
  # Random sampling without replacement:
  random.seed(4) # ensure always get the same sampled list of items every time from the specified list
  """
  # for debug only!!!
  print('$$$$$$$$$$$$$$$$$$$$$$$$')
  print('len(images) = '+str(len(images)))
  print('subset_size = ' + str(subset_size))
  print('root_real = ' + str(root_real))
  print('class_to_idx = ' + str(class_to_idx))
  assert(False)
  """
  images_subset = random.sample(images, subset_size)
  
  #print(len(images_subset)) # 1000
  #print(images_subset[5])
  #print(images_subset[99])
...
class ImageFolder(data.Dataset):
  ...
  Args:
      root (string): Root directory path. # Note: need to modify this! --> let root only contains subset?
  ...
  def __init__(self, root, transform=None, target_transform=None,
    ...
    #print(root) # for debug --> data/flower_subset_1000
    """
    # original code:
    classes, class_to_idx = find_classes(root)
    """
    # My modification: start! --> root acts as a string flag
    # eg, 'flower' v.s. 'flower_subset_1000'
    if 'subset' not in root:
      # SAME as original code:
      classes, class_to_idx = find_classes(root)
    else:
      # first get the actual dataset name:
      root_real = root.split('_subset_')[0]
      classes, class_to_idx = find_classes(root_real)
    # My modification: end!
    #assert(False) # for debug
    ...
    # results to a pre-computed file.
      ...
      if 'subset' not in root:
        ...
        #print('&&&&&&&&&&&&&&&&&&&&&&&&&&')
        #print(root) # 'data/flower_subset_1000'
        #print(class_to_idx) # {'jpg': 0}
        # get the actual dataset name:
        root_real = root.split('_subset_')[0] # 'data/flower'
        subset_size = int(root.split('_subset_')[-1]) # 1000
        #assert(False)
        imgs = make_dataset_subset(root_real, class_to_idx, subset_size)
        np.savez_compressed(index_filename, **{'imgs' : imgs})
        # May cause problem?:
        root = root_real
      # My modification: end!
...
''' LSUNbed_HDF5: A dataset to support I/O from an HDF5 to avoid
    having to load individual images all the time. '''
class LSUNBed_HDF5(data.Dataset):
  def __init__(self, root, transform=None, target_transform=None,
               load_in_mem=False, train=True,download=False, validate_seed=0,
               val_split=0, **kwargs): # last four are dummies
