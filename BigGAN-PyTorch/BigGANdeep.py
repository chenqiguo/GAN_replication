...
class Discriminator(nn.Module):
  ...
  def __init__(self, D_ch=64, D_wide=True, D_depth=2, resolution=128,
    ...
    # Prepare model
    # Stem convolution
    self.input_conv = self.which_conv(3, self.arch['in_channels'][0]) # original code for RGB
    #self.input_conv = self.which_conv(1, self.arch['in_channels'][0]) # new code modified by chenqi: for grayscale
...
class G_D(nn.Module):
  ...
  def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
    ...
    # along the batch dimension for improved efficiency.
      ...
      # for debug:
      print('**********DEBUG**********')
      print('D_input.shape = ' + str(D_input.shape))
      print('D_class.shape = ' + str(D_class.shape))
      assert(False)
      """
