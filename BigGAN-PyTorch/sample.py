...
def run(config):
  ...
  """
  # just for debug:
  print('config[\'config_from_name\'] = ' + str(config['config_from_name'])) # False
  print('config[\'weights_root\'] = ' + config['weights_root']) # 'weights'
  print('config[\'experiment_name\'] = ' + config['experiment_name']) # ''
  print('config[\'load_weights\'] = ' + config['load_weights']) # ''
  """
  ...
  """
  # just for debug:
  print('config[\'resolution\'] = ' + str(config['resolution'])) # 128
  """
  ...
  utils.load_weights(G if not (config['use_ema']) else None, None, state_dict, 
                     ...
                     strict=False, load_optim=False) # Chenqi's notes: originally load_optim=False : load the model: G_ema.pth
  ...
  if config['sample_random']:
    ...
    # newly modified: my 1st try:
    for ii in range(int(np.ceil(1024 / float(G_batch_size)))): # NOTE: 1024 is the total num of imgs in stylegan sheets!
        images, labels = sample()    
        torchvision.utils.save_image(images.float(),
                                     '%s/%s/chenqi_random_samples/random_samples%d.jpg' % (config['samples_root'], experiment_name, ii),
